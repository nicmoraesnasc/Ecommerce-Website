...
def create_user():
    if request.method == 'POST':
        ...
        database.insert_user(
            name, last_name, phone, zip_code, state, city, neighborhood,
            address, house_number, email, password, 0
        )
...
def update_user(id):
    ...
    return render_template(
        ...
        last_name=user[2],
        phone=user[3],
        email=user[6],  # Agora email está na posição 6
        state=user[4],  # state na posição 4
        city=user[5],   # city na posição 5
        zip_code=user[9],  # zip_code na posição 9
        neighborhood=user[10],  # neighborhood na posição 10
        address=user[11],  # address na posição 11
        house_number=user[12],  # house_number na posição 12
        password='',
        user_id=user[0],
        is_admin=database.is_admin(session['user_id'])
    )
...
def admin_orders():
    ...
    orders = database.get_all_orders(status_filter=status_filter, search_query=search_query)
...
def admin_order_detail(order_id):
    ...
    if request.method == 'POST':
        ...
        try:
            database.update_order_status(order_id, shipping_status, payment_status, tracking_code)
...
@app.route('/about')
def about():
    database.mycursor.execute("SELECT * FROM about WHERE id = 1")
    about_data = database.mycursor.fetchone()
    
    info = {
        'title': about_data[1] if about_data else 'Sem título',
        'content': about_data[2] if about_data else 'Sem conteúdo'
    }
    
    database.mycursor.execute("SELECT * FROM SocialMedia ORDER BY id")
    social_media_tuples = database.mycursor.fetchall()
    
    print(f"REDES DO BANCO: {social_media_tuples}")
...
def about_edit():
    database.mycursor.execute("SELECT * FROM about WHERE id = 1")
    current_info = database.mycursor.fetchone()
    ...
    if request.method == 'POST':
        ...
        database.mycursor.execute("UPDATE about SET title = %s, content = %s WHERE id = 1", (title, content))
        
        social_names = request.form.getlist('social_name[]')
        social_links = request.form.getlist('social_link[]')
        
        print(f"REDES DO FORMULÁRIO: {social_names} -> {social_links}")
        
        database.mycursor.execute("DELETE FROM SocialMedia")
        
        for name, link in zip(social_names, social_links):
            if name and link:
                database.mycursor.execute(
                    "INSERT INTO SocialMedia (social_name, social_link) VALUES (%s, %s)",
                    (name, link)
                )
...
def delete_social_media(id):
    database.mycursor.execute("DELETE FROM social_media WHERE id = %s", (id,))
    database.mydb.commit()
